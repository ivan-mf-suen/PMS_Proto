  ...
  return (
    <div style={{ padding: 24, maxWidth: 1400 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: 'var(--foreground)', letterSpacing: '-0.02em' }}>Reports</h1>
          <p style={{ fontSize: 13, color: '#64748B', marginTop: 4 }}>Analytics and reporting dashboard</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          { icon: BarChart3, label: 'Financial Summary', color: 'var(--info)' },
          { icon: TrendingUp, label: 'Work Order Analytics', color: 'var(--success)' },
          { icon: PieChart, label: 'Occupancy Report', color: '#8B5CF6' },
          { icon: Calendar, label: 'Compliance Calendar', color: '#F59E0B' },
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid var(--border)', boxShadow: 'var(--card-shadow)' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', fontSize: 15, fontWeight: 700, color: 'var(--foreground)' }}>Generated Reports</div>
        {REPORTS_DATA.map((report) => (
          <div key={report.id} style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }} onMouseEnter={(e) => (e.currentTarget.style.background = '#F8FAFC')} onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}>
            ...
            <button style={{ padding: '6px 12px', fontSize: 12, fontWeight: 600, borderRadius: 6, border: '1px solid var(--border)', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}><Download size={12} /> Export</button>
          </div>
        ))}
      </div>
    </div>
